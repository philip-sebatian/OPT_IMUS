...
This document provides detailed API reference for the Optimus routing system.
...
Both `solve_optimized()` and `solve_enhanced()` return a dictionary with the following structure:
...
```python
{
    ...
    'cost': float,                          # Cost of this assignment
...
The system uses configuration from `optimus.config.default_config`:

```python
from optimus.config.default_config import DEFAULT_CONFIG
...
See the `optimus/examples/` directory for comprehensive usage examples:
