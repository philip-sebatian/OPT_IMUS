#!/usr/bin/env python3
"""
Generate improved graph with better labeling and route information.
"""
...
def create_improved_test_data():
    """Create test data with clear routes for better visualization."""
    print("Creating improved test data for visualization...")
    ...
    # 2. Create vehicles with clear starting positions
    vehicles = []
    vehicle_configs = [
    ...
    # 4. Create clear optimization results with realistic routes
    mock_assignments = []
    mock_vehicle_routes = {}
    ...
    # Assign tasks to vehicles with clear routes
    assignments = [
        (0, 3, 'direct', 25),      # Vehicle 0 -> Store-3
        (1, 4, 'direct', 30),      # Vehicle 1 -> Store-4
        (2, 5, 'direct', 20),      # Vehicle 2 -> Store-5
        (0, 6, 'refill', 35),      # Vehicle 0 -> Store-6 (after refill)
        (1, 7, 'split', 15),       # Vehicle 1 -> Store-7 (split with Vehicle 2)
    ]
    
    for i, (vehicle_id, store_id, strategy, demand) in enumerate(assignments):
        assigned_vehicle = vehicles[vehicle_id]
        
        assignment_cost = random.uniform(20, 60)
...
def generate_improved_graph():
    """Generate the improved graph visualization."""
    print("="*60)
    print("GENERATING IMPROVED OPTIMUS ROUTING GRAPH")
    print("="*60)
    
    # Create test data
    vehicles, delivery_tasks, depot_manager, results = create_improved_test_data()
    ...
    try:
        filename = create_static_visualization(
            ...
            "optimus_improved_graph.png"
        )
        print(f"✅ SUCCESS! Improved graph saved to: {filename}")
        ...
        print("- Route text boxes: Complete path for each vehicle")
