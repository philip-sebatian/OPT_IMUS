#!/usr/bin/env python3
"""
Test script for the fixed pretty printer.
"""
...
def run_fixed_pretty_printer_test():
    print("--- Testing Fixed Pretty Printer ---")
    ...
    for i, task in enumerate(delivery_tasks):
        ...
        strategy_choice = random.choice(['direct', 'refill', 'split'])
        
        assignment_cost = random.uniform(10, 100)
        ...
        if strategy_choice == 'split':
            ...
            if vehicle_id_2 == assigned_vehicle.id:
        ...
        assigned_vehicle.current_stock -= task.demand
        assigned_vehicle.position = nearest_depot_loc

        if strategy_choice == 'split':
            if 'vehicle_id_2' in locals():
    ...
    # 5. Test the fixed pretty printer
